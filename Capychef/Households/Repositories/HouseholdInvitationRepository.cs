...
public class HouseholdInvitationRepository(CapychefDbContext dbContext) : IHouseholdInvitationRepository
{
    ...
    {
        return await dbContext.HouseholdInvitations.Active().AsNoTracking()
            .Where(x => x.UserId == userId && !x.IsAnswered)
            .Include(x => x.User)
            .Include(x => x.Creator)
            .Include(x => x.Household).ToListAsync();
    }
    ...
    public async Task<HouseholdInvitation?> GetTrackedInvitationByIdAndHouseholdId(int invitationId, int householdId)
    {
        return await dbContext.HouseholdInvitations.Active()
            .Where(x => x.Id == invitationId && x.HouseholdId == householdId)
            .FirstOrDefaultAsync();
    }
